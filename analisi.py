...
def load_video(path, max_frames=0, resize=(R, C)):
    Temp = np.load(path)['Temp']

    min = Temp.min()
    max = Temp.max()

    Temp = 255*(Temp-min)/max

    frames = []
    len = Temp.shape[2]
    
    for i in range(len):
        frame = np.dstack((Temp[:,:,i], Temp[:,:,i], Temp[:,:,i]))
...
# train_data, train_diams = prepare_all_videos(train_df, "train")
